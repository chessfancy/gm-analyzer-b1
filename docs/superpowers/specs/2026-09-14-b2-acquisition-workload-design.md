...
3. **Canonical identity is deduplicated.** Multiple source occurrences may point to one canonical game.
4. **Analysis happens once per canonical game content.** Duplicate occurrences do not trigger duplicate Stockfish analysis.
...
The registry stores both the score and reasons used to produce it. Vietnamese detection starts with PGN federation headers (`VIE`) and a small alias table, with room for later FIDE identity mapping.
...
- omit exact duplicate canonical games from tournament workload output,
...
Only changed canonical game content should require future re-analysis.
...
Sharding metadata records game IDs/fingerprints included in each shard so a later result can always be mapped back to the canonical registry.
...
- Duplicate raw files are stored/referenced by SHA without creating duplicate canonical analysis work.
...
- job spec: `cgm-job-1`,
- sharding policy version: `whole_game_plies_v1`.
...
6. canonical PGN generation is deterministic and parseable;
7. Vietnamese priority detection recognizes `VIE` federation and records reasons;
8. tournament priority scoring is deterministic and explainable;
9. whole-game sharding never splits a game and is deterministic;
10. sharding balances on plies within policy expectations;
11. `TournamentManifest` and `JobSpec` canonical JSON/hash outputs are deterministic;
12. `JobSpec` contains B1 v3 analysis policy but no provider/workers/threads/Hash;
13. Local object-store round trips and checksum verification pass;
14. S3 object-store behavior is covered through a fake/stub client without requiring network credentials in the normal test suite;
15. an emitted shard PGN can be consumed by the existing B1 import/analyzer pipeline without modifying B1 semantics;
16. all existing B1 tests remain green.

## Delivery sequence

### B2.1 — Contracts and identity

Implement canonical JSON utilities, game fingerprinting, `TournamentManifest`, `JobSpec`, object-key helpers, and tests.

### B2.2 — Registry

Implement `registry.sqlite` schema, idempotent migrations, source/tournament/file/game-occurrence persistence, metadata conflicts, and priority reasons.
...
If the same chess games are later encountered through another source, raw occurrences are preserved but exact duplicate canonical games do not create duplicate shards or duplicate B1 analysis work.
